...
Map<String, List<Student>> Students = new LinkedHashMap<String, List<Student>>();
...
List<Student> Selenium = new ArrayList<Student>();
      ...
      List<Student> QTP = new ArrayList<>();

      ConnectDB connectData = new ConnectDB();
      for(Object SelInfo:Selenium){
         connectData.InsertDataFromStringToMySql(SelInfo.toString(), "StudentsDataBase", "StudentInfo");
      }

      for(Object SelInfo:QTP){
         connectData.InsertDataFromStringToMySql(SelInfo.toString(), "StudentsDataBase", "StudentInfo");
      }
...
Selenium = xmlReader.parseData(tag, pathSelenium);
...
Students.put("QTP", QTP);

      //Retrieve map data and display output.

Iterator StudentList = Students.entrySet().iterator();// enter to iterator through map.entrySet

for (Map.Entry<String, List<Student>> StudentInfo: Students.entrySet()) {

         System.out.print(StudentInfo.getKey() + " " + StudentInfo.getValue() + " " + "grade:" + "\n");
      }
	...
	private static class ConnectDB {
		public void InsertDataFromStringToMySql(String s, String studentsDataBase, String studentInfo) {
		}
	}
}